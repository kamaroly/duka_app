defmodule DukaApp.HomeScreen do
  @moduledoc "Landing screen for DukaApp."
  use Mob.Screen
  ...
  def render(assigns) do
    ...
    <Scroll background={:background}>
      <Column background={:background} padding={:space_lg}>
        ...
        <Spacer size={16} />
        <Text text="DukaApp" text_size={:xl} text_color={:on_surface} padding={:space_sm} />
      ...
      <TabBar
        active={to_string(:main)}
        on_tab_select={on_select}
        background={:surface}
        padding={1}
        tabs={[
        %{id: :home,  label: "Home",  icon: "house"},
        %{id: :inventory,  label: "Inventory",  icon: "doc.text"},
        %{id: :profile, label: "Profile", icon: "waveform"}
      ]}
      />
    </Scroll>
    """
  ...
  def handle_info({:tap, :open_text}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.TextScreen)}
  end

  def handle_info({:tap, :open_list}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.ListScreen)}
  end

  def handle_info({:tap, :open_dice}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.DiceScreen)}
  end

  def handle_info({:tap, :open_webview}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.WebViewScreen)}
  end

  def handle_info({:tap, :open_audio}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.AudioScreen)}
  end

  def handle_info({:tap, :open_storage}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DukaApp.StorageScreen)}
  end
