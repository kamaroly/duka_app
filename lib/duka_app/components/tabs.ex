defmodule DukaApp.Components.Tabs do
  @moduledoc """
  ...
  Write them out as `<Tab>` children. A tab and its panel are declared
  together rather than matched up by position:

      <Tabs active={@tab} on_change={:pick} id="main">
  ...
  alias DukaApp.Components.Event

  @tab_type :mishka_tab

  @doc """
  Composite expander (`<Tabs>`). Children are `:mishka_tab` nodes.
  """
  ...
  The test tag on the scrolling strip, given the tabs' `id`.
      ...
      iex> DukaApp.Components.Tabs.strip_id("plans")
      "plans-strip"
  """
  ...
  cannot tell the selected tab from any other.
      ...
      iex> DukaApp.Components.Tabs.tab_id("plans", :pro, :active)
  ...
  The test tag on the panel below the strip.
      ...
      iex> DukaApp.Components.Tabs.panel_id("plans")
      "plans-panel"
  """
